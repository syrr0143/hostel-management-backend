...
router.post ('/AdminSignup',async(req,res)=>{
    try {
        ...
        const {name ,contactInformation, password , emailAddress,address,gender,hostelId } = req.body;

        // check if the emailAddress is already used for signup process 
        const existingAdmin = await Admin.findOne({emailAddress});

        if(existingAdmin){
            return res.status(400).json({error:"Username with same email already have an account , login instead", existingAdmin});

        }

        //create a new user if email is not taken already 
        const hashedPassword = await bcrypt.hash(password, 10);
        const newAdmin = new Admin ({name,contactInformation ,password:hashedPassword , emailAddress,address, gender,hostelId });
...
router.post ('/UserSignup',async(req,res)=>{
    try {
        console.log("request received for signup :", req.body);
        const {FullName , Password ,DOB,RollNumber,Branch,RoomAlloted,contactNumber,City,State,gender,EmailAddress,hostelId } = req.body;
        ...
        const newUser = new User ({FullName , Password:hashedPassword ,DOB,RollNumber,Branch,RoomAlloted,contactNumber,City,State,gender,EmailAddress,hostelId });
